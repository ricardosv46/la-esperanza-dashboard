...
const CreateBlog = () => {
  ...
  const { db: dataCategoriaBlog, loading: locadingCategoria } = useCategoriaBlogs({ estado: 'Activado' })
  ...
  const [imagenSecundaria, setImagenSecundaria] = useState<Imagenes>({} as Imagenes)
  const navigate = useNavigate()

  const { titulo, descripcionCorta, descripcionLarga, keywords, onChange } = useForm({
    titulo: '',
    descripcionCorta: '',
    descripcionLarga: '',
    keywords: ''
  ...
  useEffect(() => {
    if ([titulo, descripcionCorta, descripcionLarga, keywords].includes('') || selectValue === 0) {
      setIsDisable(true)
    } else {
      setIsDisable(false)
    }
        ...
        <Box maxWidth={'full'}>
          {locadingCategoria ? (
            ...
            <Grid mt={5} templateColumns={{ base: 'repeat(1, 1fr)', sm: 'repeat(2, 1fr)' }} gap={6}>
              <InputFloat type='text' label='Titulo' name='titulo' value={titulo} onChange={onChange} />
              <InputFloat type='text' label='Keywords' name='keywords' value={keywords} onChange={onChange} />
              <InputFloat
                type='text'
              ...
              <Box></Box>
              <InputImage value={imagenPrincipal} onChange={setImagenPrincipal} label=' Imagen Principal' />
              <InputImage value={imagenSecundaria} onChange={setImagenSecundaria} label=' Imagen Secundaria' />
            </Grid>
          )}
        </Box>
      </Flex>
      <Flex justifyContent='center' py={14}>
        <Button type='button' w={96} py={7} colorScheme='primary' onClick={handleSubmit} disabled={isDisable}>
