			...
			<Flex flexDir={'column'}>
				<Box maxWidth={'full'}>
					...
					<Text color='blackAlpha.600' _dark={{ color: 'gray.400' }} fontSize='14px' mt={3}>
						Desde aquí podrás visualizar la información del precio del abono.
					</Text>
				</Box>
								...
								<Tbody>
									{butacas.map((butaca) => (
										<Tr key={butaca?.butacaId}>
											<Td>{butaca?.codigo}</Td>
											<Td>{String(butaca?.precio).includes('.') ? butaca?.precio : `${butaca?.precio}.00`}</Td>
