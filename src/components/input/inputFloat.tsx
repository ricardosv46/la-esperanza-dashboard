...
const InputFloat = ({ label, name, type, value, onChange, errorMessage, rightElement, maxLength }: IProps) => {
  ...
  return (
    <FormControl
      ...
      <Input
        ...
        _focus={{ boxShadow: '0 0 0 1px #00B593', borderBottomColor: 'primary.500' }}
      />
      <FormLabel
        htmlFor={name}
        ...
        fontSize={type === 'date' || type === 'time' || value?.length !== 0 ? 'sm' : 'md'}>
        {label}
      </FormLabel>
      {/* eslint-disable */}
      {type === 'password' ? (
